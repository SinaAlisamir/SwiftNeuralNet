...
// DEMO -----------------------------------------------------------------
func NeuralNetwork_Demo() {
    let input = matrix()
    input.matrix = [[0.4,0.1] , [0.2,0.5] , [0.3,0.6]]
    ...
    for _ in 0...200 {
        let rand1 = math().rand()
        let rand2 = math().rand()
        input.matrix.append([rand1,rand2])
        if rand1>rand2 {
            output.matrix.append([1])
        }else{
            output.matrix.append([0])
        }
    }

    let mlp = NeuralNetwork(numberOfUnits: [2,5,1])
    mlp.step = 5
    mlp.train(input: input, output: output, iteration_num: 100)

    let test = matrix()
    test.matrix = [[0.4,0.1]]
    print("output: \(mlp.output_for(input: test)[0][0]) prediction: \(round(mlp.output_for(input: test)[0][0])) real: 1")

    test.matrix = [[0.2,0.1]]
    print("output: \(mlp.output_for(input: test)[0][0]) prediction: \(round(mlp.output_for(input: test)[0][0])) real: 1")

    test.matrix = [[0.5,0.8]]
    print("output: \(mlp.output_for(input: test)[0][0]) prediction: \(round(mlp.output_for(input: test)[0][0])) real: 0")

    test.matrix = [[0.62,0.65]]
    print("output: \(mlp.output_for(input: test)[0][0]) prediction: \(round(mlp.output_for(input: test)[0][0])) real: 0")
}
// ----------------------------------------------------------------------
